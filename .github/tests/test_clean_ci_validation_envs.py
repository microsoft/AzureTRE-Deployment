"""Test cleanup decisions using mocked Azure, GitHub, Git and date commands."""
...
CLEANUP_SCRIPT = Path(__file__).resolve(
).parents[1] / "scripts/clean_ci_validation_envs.sh"
...
if command == "gh":
    if args[0] == "api":
        ...
        counts = (
            json.loads(counts_file.read_text()) if counts_file.exists() else {}
        )
        counts[status] = counts.get(status, 0) + 1
        counts_file.write_text(json.dumps(counts))
        page_key = (
            "later_pages"
            if counts[status] > 1 and "later_pages" in config else "pages"
        )
    ...
    if args[:2] == ["group", "list"]:
        ...
        rows = (
            config["groups"] if "ci_git_ref" in query else ["rg-main-ws-old"]
        )
        for row in rows:
            print(row)
    ...
    print(
        2000000000 - config["age_hours"] * 3600 if "-d" in args else 2000000000
    )
...
class CleanupTests(unittest.TestCase):
    def setUp(self):
        ...
        # Use only mocks and jq/cut, with no credentials in the subprocess.
        self.env = {
            "PATH": str(bin_dir),
            "HOME": str(self.root),
    ...
    def workflow_run(run_id=456, branch="main"):
        return {
            ...
            "pull_requests": [],
            "html_url": (
                f"https://github.com/example/tre/actions/runs/{run_id}"
            ),
        }
    ...
    def test_comment_triggered_pr_run_protects_stale_environment(self):
        for status in (
            ...
            with self.subTest(status=status):
                self.config["pages"] = {
                    status: [{"workflow_runs": [self.workflow_run()]}]}
    ...
    def test_active_run_on_another_branch_also_blocks_cleanup(self):
        self.config["pages"] = {"in_progress": [
            {"workflow_runs": [self.workflow_run(branch="feature/test")]}]}
        result = self.run_cleanup()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assert_no_azure_calls()
    ...
    def test_github_errors_prevent_all_cleanup(self):
        for status in (
            ...
            with self.subTest(status=status):
                ...
                self.assertIn(
                    "Could not check active workflow runs", result.stderr)
                self.assert_no_azure_calls()
    ...
    def test_invalid_or_empty_github_response_prevents_cleanup(self):
        for response in ("not json", "{}", ""):
            with self.subTest(response=response):
                self.config.update(
                    bad_response_status="in_progress", bad_response=response)
                result = self.run_cleanup()
                self.assertNotEqual(result.returncode, 0)
                self.assert_no_azure_calls()
    ...
    def test_malformed_recheck_prevents_main_workspace_cleanup(self):
        ...
        self.assertIn(["control_tre.sh", "stop"], self.calls)
        self.assertFalse(any(call[:3] == ["az", "group", "delete"]
                         for call in self.calls), self.calls)
    ...
    def test_current_run_allows_stop_and_workspace_cleanup(self):
        self.config["pages"] = {"in_progress": [
            {"workflow_runs": [self.workflow_run(run_id=123)]}]}
        result = self.run_cleanup()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(["control_tre.sh", "stop"], self.calls)
        self.assertIn(["az", "group", "delete", "--yes",
                      "--no-wait", "--name", "rg-main-ws-old"], self.calls)

    def test_new_run_blocks_main_workspace_cleanup(self):
        self.config["later_pages"] = {"in_progress": [
            {"workflow_runs": [self.workflow_run()]}]}
        result = self.run_cleanup()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(["control_tre.sh", "stop"], self.calls)
    ...
    def test_idle_cleanup_still_destroys_expired_environments(self):
        ...
        result = self.run_cleanup()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(["destroy_env_no_terraform.sh",
                      "--core-tre-rg", "rg-tretest", "--no-wait"], self.calls)
    ...
    def test_idle_cleanup_still_destroys_removed_branch_environments(self):
        self.config.update(
            groups=["rg-tretest\trefs/heads/deleted"], missing_branch=True)
        result = self.run_cleanup()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(["destroy_env_no_terraform.sh",
                      "--core-tre-rg", "rg-tretest", "--no-wait"], self.calls)
